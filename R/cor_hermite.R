# =============================================================================
# cor_hermite.R -- Distribution-Robust Pearson Correlation
# =============================================================================
...
#' Supports both a \strong{copula-free} empirical cross-moment formulation
#' (\code{copula = "none"}, default) and a \strong{parametric Gaussian-copula}
#' formulation via Mehler's bilinear expansion (\code{copula = "gaussian"}).
#'
#' Unlike rank correlations (Spearman's \eqn{\rho}, Kendall's \eqn{\tau}) or Gaussian rank
#' correlation, \eqn{r_{\mathrm{Hermite}}} preserves the raw manifest Pearson correlation
#' estimand without altering the metric or discarding authentic tail variance.
#'
#' @param x A numeric vector, numeric \code{\link[base]{matrix}}, or \code{\link[base]{data.frame}}.
#' @param y A numeric vector, required when \code{x} is a vector; ignored when \code{x} is a
...
#'   correlation \eqn{\rho_z} to the result. Default is \code{FALSE}.
...
#'         the sample cross-moments, which is algebraically identical to the sample covariance of
#'         the monotonically smoothed reconstructions
#'         \eqn{\hat{w} = (f(Z_x) - \mu_x)(g(Z_y) - \mu_y)}.
#'   \item \strong{Gaussian Copula (\code{copula = "gaussian"}):} Evaluates the expectation
#'         analytically via Mehler's (1866) expansion
#'         \eqn{\mathbb{E}[He_k(Z_x)He_l(Z_y)] = \delta_{kl} k! \rho_z^k}, yielding
#'         \eqn{\operatorname{Cov}(X, Y) = \sum_{k=1}^{\min(d_x, d_y)} a_k b_k k! \rho_z^k}.
#' }
#' }
#'
...
#' asymmetry (Hoeffding-Frechet bounds). For \code{copula = "none"}, \eqn{\rho_z} and \eqn{A}
#' are not defined.
#' }
#'
#' @return
...
#'   \item{\code{var_x}, \code{var_y}}{Regularized manifest variances of \eqn{X} and \eqn{Y}.}
#'   \item{\code{mean_x}, \code{mean_y}}{Regularized manifest means of \eqn{X} and \eqn{Y}.}
...
#'   \item{\code{cov}}{The \eqn{p \times p} regularized covariance matrix (variances on the diagonal).}
#'   \item{\code{marginals}}{A data frame of regularized univariate summary statistics per column.}
...
#'
#' Hermite, C. (1866). Sur un nouveau developpement en serie des fonctions. \emph{Comptes Rendus de l'Academie des Sciences, Paris}, 58, 93-100.
#'
#' Lenhard, W., & Lenhard, A. (2026). The Hermite Correlation: A Distribution-Robust Estimator of the Pearson Correlation Coefficient. \emph{Behavior Research Methods}.
#'
#' Mehler, F. G. (1866). Ueber die Entwicklung einer Function von beliebig vielen Variabeln nach Laplaceschen Functionen hoeherer Ordnung. \emph{Journal fuer die reine und angewandte Mathematik}, 66, 161-176. \doi{10.1515/crll.1866.66.161}
#'
#' @seealso \code{\link{pcor_hermite}}, \code{\link{d_reg}}, \code{\link{hermite_fit}},
#'   \code{\link{cor_hermite_boot_ci}}, \code{\link{hermite_test}}
#'
#' @examples
...
# -----------------------------------------------------------------------------
# Internal Helpers: Pairwise Kernel
# -----------------------------------------------------------------------------

#' Degenerate pairwise result (shared NA scaffold)
#' @noRd
.cor_pair_empty <- function(var_x, var_y, mean_x, mean_y, dx, dy) {
  list(
    r_Hermite       = NA_real_,
    rho_z           = NA_real_,
    ...
    degrees         = c(x = dx, y = dy),
    cross_moments   = NULL,
    rho_z_reference = NULL
  )
}

#' Pairwise Hermite covariance and correlation kernel
#'
#' Computes the regularized covariance and correlation for two fitted quantile
#' models, either from empirical cross-moments of the fitted reconstructions
#' (copula-free) or in closed form via Mehler's bilinear expansion (Gaussian
#' copula). Fitted reconstructions are obtained through
#' \code{\link{predict.hermite_fit}}, the canonical polynomial evaluator.
#'
#' @noRd
.cor_hermite_pair <- function(fit_x, fit_y, copula = "none", trim = 0,
                              diagnostics = FALSE) {
  ...
  if (is.na(var_x) || is.na(var_y) || var_x <= 0 || var_y <= 0) {
    return(.cor_pair_empty(var_x, var_y, mean_x, mean_y, dx, dy))
  }
  ...
  if (copula == "none") {
    ...
    w <- (predict(fit_x) - mean_x) * (predict(fit_y) - mean_y)
    ...
    if (diagnostics) {
      ...
      dimnames(cross_moments) <- list(paste0("He", seq_len(dx)),
                                      paste0("He", seq_len(dy)))
    ...
    bx_pad <- rep(0.0, max_d + 1L); bx_pad[seq_len(dx + 1L)] <- fit_x$beta
    by_pad <- rep(0.0, max_d + 1L); by_pad[seq_len(dy + 1L)] <- fit_y$beta
...
cor_hermite.default <- function(x, y, poly_degree = 3L,
  ...
  if (n < 4L || diff(range(x_c)) == 0 || diff(range(y_c)) == 0) {
    ...
    return(structure(
      list(r_Hermite = NA_real_, copula = copula,
           rho_z = NA_real_, attenuation = NA_real_),
      class = "cor_hermite"
    ))
  }
...
cor_hermite.matrix <- function(x, poly_degree = 3L,
  ...
  R_copula <- if (gaussian) R_Hermite else NULL
  Att_mat  <- if (gaussian) R_Hermite else NULL

  # Marginal fits (each column fitted once on its complete cases)
  fits    <- vector("list", p)
  means_v <- numeric(p); vars_v <- numeric(p); sds_v <- numeric(p)
  skew_v  <- numeric(p); degs_v <- integer(p)

  fully_complete <- all(is.finite(x))

  for (i in seq_len(p)) {
    xi <- if (fully_complete) x[, i] else x[is.finite(x[, i]), i]
    fits[[i]] <- hermite_fit(xi, degree = poly_degree, monotonicity = monotonicity,
                             ties_method = ties_method, force_odd = TRUE)
  ...
  for (i in 1:(p - 1L)) {
    for (j in (i + 1L):p) {
      if (fully_complete) {
        ...
        # Pairwise complete fallback: refit on shared complete cases
        pair <- cor_hermite.default(x[, i], x[, j], poly_degree = poly_degree,
...
#' two variables controlling for one or more covariates, or the full partial
#' correlation matrix for a multivariate dataset via precision matrix inversion.
#'
...
#'   where \code{z} is partialled out of \code{y} only. Default is \code{FALSE}
#'   (partial correlation).
...
#' @param monotonicity Monotonicity constraint: \code{"relaxed"} (default),
#'   \code{"strict"}, or \code{"none"}.
#' @param conf_level Numeric value in \eqn{(0, 1)} for confidence intervals, or
#'   \code{NULL} (default).
#' @param ci_method Method for CI: \code{"fisher"} (analytical Fisher \eqn{z}) or
#'   \code{"bootstrap"}.
...
#'
#' @return
#' If \code{x} is a matrix or data frame (and \code{y} is \code{NULL}), an object of
#' class \code{"pcor_hermite_matrix"} containing:
#' \describe{
...
#' }
#'
#' If \code{x}, \code{y}, and \code{z} are supplied, an object of class
#' \code{"pcor_hermite"} containing:
#' \describe{
...
#'   \item{\code{semi}}{Logical; whether a semipartial correlation was computed.}
#'   \item{\code{copula}}{The copula mode applied.}
...
pcor_hermite <- function(x, y = NULL, z = NULL,
  ...
  if (is.matrix(x) || is.data.frame(x)) {
    ...
    inv_R <- tryCatch(
      solve(R),
      error = function(e) solve(R + diag(1e-4, nrow = p, ncol = p))
    )
    ...
    return(structure(
      list(pcor = P_mat, r_Hermite = R, cov = fit_mat$cov,
           copula = copula, n = fit_mat$n),
      class = "pcor_hermite_matrix"
    ))
  }
  ...
  # Partition: block 1 = (X, Y), block 2 = Z; Schur complement conditioning
  idx_z <- 3:(2 + k)
  S11 <- Sigma[1:2, 1:2, drop = FALSE]
  S12 <- Sigma[1:2, idx_z, drop = FALSE]
  S22 <- Sigma[idx_z, idx_z, drop = FALSE]

  inv_S22 <- tryCatch(
    solve(S22),
    error = function(e) solve(S22 + diag(1e-4, nrow = k, ncol = k))
  )
  S_cond <- S11 - S12 %*% inv_S22 %*% t(S12)

  denom <- if (semi) sqrt(S11[1, 1] * S_cond[2, 2]) else sqrt(S_cond[1, 1] * S_cond[2, 2])
  pcor_val <- if (is.na(denom) || denom <= 0) NA_real_ else {
    max(-1.0, min(1.0, S_cond[1, 2] / denom))
  }

  # Latent copula partial correlation (Gaussian mode only)
  rho_z_pcor <- NA_real_
  att_val    <- NA_real_
  if (copula == "gaussian" && !is.null(fit_mat$rho_z)) {
    R_z <- fit_mat$rho_z
    R11 <- R_z[1:2, 1:2, drop = FALSE]
    R12 <- R_z[1:2, idx_z, drop = FALSE]
    R22 <- R_z[idx_z, idx_z, drop = FALSE]

    inv_R22 <- tryCatch(
      solve(R22),
      error = function(e) solve(R22 + diag(1e-4, nrow = k, ncol = k))
    )
    R_cond  <- R11 - R12 %*% inv_R22 %*% t(R12)
    denom_z <- sqrt(R_cond[1, 1] * R_cond[2, 2])
    rho_z_pcor <- if (is.na(denom_z) || denom_z <= 0) NA_real_ else {
      max(-1.0, min(1.0, R_cond[1, 2] / denom_z))
    }
    att_val <- if (is.finite(rho_z_pcor) && abs(rho_z_pcor) > 1e-6) {
      pcor_val / rho_z_pcor
    } else 1.0
  }
  ...
  if (!is.null(conf_level) && is.finite(pcor_val)) {
    if (ci_method == "fisher") {
      ...
      for (b in seq_len(B)) {
        ...
        boot_vals[b] <- tryCatch(
          pcor_hermite(x = df_all$X[idx], y = df_all$Y[idx],
                       z = df_all[idx, idx_z, drop = FALSE],
                       semi = semi, copula = copula, poly_degree = poly_degree,
                       monotonicity = monotonicity)$estimate,
...
#' Convenience Alias for the Gaussian-Copula Hermite-Mehler Correlation
#' @rdname cor_hermite
#' @export
...
# -----------------------------------------------------------------------------
# Inference: Bootstrap CI and confint
# -----------------------------------------------------------------------------

#' Percentile Bootstrap Confidence Interval for the Hermite Correlation
#'
#' Non-parametric percentile bootstrap for a fitted \code{"cor_hermite"}
#' object, refitting the entire pipeline (NQT, monotone quantile models,
#' covariance) on every resample.
#'
...
#' @seealso \code{\link{cor_hermite}}, \code{\link{confint.cor_hermite}}
#' @export
cor_hermite_boot_ci <- function(object, x = NULL, y = NULL,
  ...
  empty_result <- function(n_success, boot_vals) {
    structure(
      list(estimate = object$r_Hermite, lower = NA_real_, upper = NA_real_,
           conf = conf, B = B, n_success = n_success, n_fail = B - n_success,
           success_rate = n_success / B, poly_degree = poly_degree,
           copula = copula, ties_method = ties_method, trim = trim,
           boot_estimates = boot_vals),
      class = "cor_hermite_boot_ci"
    )
  }

  if (n < poly_degree + 2L || diff(range(x)) == 0 || diff(range(y)) == 0) {
    return(empty_result(0L, numeric(0)))
  }
  ...
  if (success_rate < min_success) {
    ...
    return(empty_result(n_success, boot_estimates[success]))
  }

  alpha  <- (1 - conf) / 2
  limits <- stats::quantile(boot_estimates[success],
                            probs = c(alpha, 1 - alpha), names = FALSE, type = 7)

  structure(
    list(estimate = object$r_Hermite, lower = limits[1L], upper = limits[2L],
         conf = conf, B = B, n_success = n_success, n_fail = B - n_success,
         success_rate = success_rate, poly_degree = poly_degree, copula = copula,
         ties_method = ties_method, trim = trim,
         boot_estimates = boot_estimates[success]),
    class = "cor_hermite_boot_ci"
  )
}
...
#'   \describe{
#'     \item{\code{"fisher"}}{(Default) Fast analytical Fisher \eqn{z}-transformation
#'       with asymptotic standard error \eqn{\mathrm{SE} = 1/\sqrt{n - 3}}.}
#'     \item{\code{"bootstrap"}}{Non-parametric percentile bootstrap refitting the full
#'       Hermite pipeline across resamples; see \code{\link{cor_hermite_boot_ci}}.}
#'   }
...
confint.cor_hermite <- function(object, parm, level = 0.95,
  ...
  matrix(ci, nrow = 1L,
         dimnames = list("r_Hermite", c(paste0(100 * (1 - level) / 2, " %"),
...
print.cor_hermite <- function(x, digits = 3L, ...) {
  copula_label <- if (x$copula == "gaussian") {
    "Gaussian Copula (Mehler's identity)"
  } else {
    "Copula-Free (empirical cross-moments)"
  }

  cat("\n  Distribution-Robust Hermite Correlation\n")
  cat(strrep("-", 52), "\n", sep = "")
  cat(sprintf("  Hermite Correlation (r)    :  %.*f\n", digits, x$r_Hermite))
...
print.cor_hermite_matrix <- function(x, digits = 3L, ...) {
  ...
  marg <- x$marginals
  num_cols <- c("Mean", "Variance", "SD", "Skewness")
  marg[num_cols] <- lapply(marg[num_cols], round, digits = digits)
...
print.pcor_hermite_matrix <- function(x, digits = 3L, ...) {
  cop_lab <- if (x$copula == "gaussian") "Gaussian Copula" else "Copula-Free"
  cat(sprintf("\n  Hermite Partial Correlation Matrix (%s):\n", cop_lab))
  cat(strrep("-", 56), "\n", sep = "")
...
print.cor_hermite_boot_ci <- function(x, digits = 3L, ...) {
  cat("\n  Bootstrap Confidence Interval for the Hermite Correlation\n")
  cat(strrep("-", 52), "\n", sep = "")
